...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Security Popup Logic
    const securityPopup = document.getElementById('security-popup');
    const closeSecurityBtn = document.getElementById('close-security-popup');
        ...
        if (initialLoader) {
            setTimeout(() => {
                ...
                setTimeout(() => { 
                    ...
                    // Show popup after loader is done
                    if (securityPopup) {
                        setTimeout(() => {
                            securityPopup.classList.remove('opacity-0', 'pointer-events-none');
                            const popupContent = securityPopup.querySelector('div');
                            if (popupContent) {
    ...
    if (closeSecurityBtn && securityPopup) {
        closeSecurityBtn.addEventListener('click', () => {
            securityPopup.classList.add('opacity-0', 'pointer-events-none');
            const popupContent = securityPopup.querySelector('div');
            if (popupContent) {
